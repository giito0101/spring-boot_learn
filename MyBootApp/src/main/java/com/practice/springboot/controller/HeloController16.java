...
import com.practice.springboot.repositories.MyData3Repository;

@Controller
public class HeloController16 {
	@Autowired
	MyData3Repository repository;
	...
	public ModelAndView form(
		...
		if(!result.hasErrors()){
			repository.saveAndFlush(mydata3);
	...
	public ModelAndView index(ModelAndView mav,@PathVariable int id,@ModelAttribute("formModel") MyData3 mydata3){
		...
		mav.addObject("title","edit mydata3.");
	...
	public ModelAndView delete(@PathVariable int id,ModelAndView mav){
		...
		mav.addObject("title","delete mydata3.");
		Optional<MyData3> data=repository.findById((long)id);
		mav.addObject("formModel",data.get());
		return mav;
	}
